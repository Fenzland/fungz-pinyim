...
對普通話中不同音，而古漢語中同音的字保持區分，保持對普通話的可回歸性（除聲調外）。
...
iong  ->  üong
```
精細化之後，我們就能表達不同的口音。比如“難”在普通話中發 `nən` 而在東北話中，則發 `nan` 。“定”在普通話中爲 `ding` ，北京話中則爲 `dieng` 。
...
該讀音也不同於任何時候的古音，而是現代讀音和古音的某種結合。具有更大的讀音區分度（目前的進度，區分度仍不及古音）。
